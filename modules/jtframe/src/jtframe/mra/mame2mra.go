...
func mra_disclaimer(machine *MachineXML, year string) string {
	...
	disc.WriteString("<!--          FPGA arcade hardware for arcfpga, by bmo00\n")
	disc.WriteString(`
              Built on JTFRAME (GPLv2) by Jose Tejada / Jotego
              https://github.com/bmo00/arcfpga-cores

              This core is available for hardware compatible with MiST and MiSTer
              Other FPGA systems may be supported by the time you read this.
              This work is not maintained by the MiSTer project. Please contact the
...
func make_about_node(root *XMLNode, global GlobalCfg) {
	...
	n.AddAttr("author", notEmpty(slice2csv(global.Author), "arcfpga"))
	// No arcfpga-wide webpage/twitter to default to (unlike jotego's own Patreon/Twitter) — only
	// emit these if a core's cfg/mame2mra.toml [global] section sets them explicitly.
	if global.Webpage != "" {
		n.AddAttr("webpage", global.Webpage)
	}
	if global.Twitter != "" {
		n.AddAttr("twitter", global.Twitter)
	}
	n.AddAttr("source", "https://github.com/bmo00/arcfpga-cores")
}
